...
def month_day_report(dataset, year):
    ...
    for day, hash, average, month_name, date in dataset[['day', 'hash', 'average', 'month_name', 'date']].values:
        if date not in dates:
            report.append({'total': round(date_hash_sum.get(date), 2), 'average': round(date_average_sum.get(date), 3), 'date': f'{month_name[0:3]}. {day}, {year}'})
            dates.append(date)
...
def year_quarter_month_report(dataset, quarter_groups, months_sum, quarter_sum, year):
    ...
    for quarter in quarter_groups:
        for month_name in dataset.loc[dataset.quarter == quarter[0]].month_name.unique():
            report.append({'type': 'month', 'date': f'{month_name}', 'total': round(months_sum.get(month_name), 2)})

        report.append({'type': 'quarter', 'date': f'{toRoman(quarter[0])} Quarter', 'total': round(quarter_sum.get(quarter[0]), 2)})
        ...
        for month_name in dataset.loc[dataset.quarter == quarter[0]].month_name.unique():
            for day_ds in dataset.loc[dataset.month_name == month_name][['day', 'hash', 'month_name', 'average', 'date']].values:
                if day_ds[4] not in dates:
                    report.append({'type': 'day', 'date': f'{day_ds[2][0:3]}. {day_ds[0]}, {year}', 'average': round(date_average_sum.get(day_ds[4]), 3),
                                   'hash': round(date_hash_sum.get(day_ds[4]), 2)})
                    dates.append(day_ds[4])

            report.append({'type': 'month', 'date': f'{month_name}', 'total': round(months_sum.get(month_name), 2),
                           'average': round(months_sum_average.get(month_name), 3)})

        report.append({'type': 'quarter', 'date': f'{toRoman(quarter[0])} quarter', 'total': round(quarter_sum.get(quarter[0]), 2), 'average': round(quarter_sum_average.get(quarter[0]), 3)})
...
def quarter_month_report(dataset, month_sums, month_names, year, quarter):
    ...
    for (month_pk, month_sum), month_name in zip(month_sums.items(), month_names):
        report.append({'date': month_name, 'total': round(month_sum, 2)})

    return {'report': report, 'total': round(float(dataset.hash.sum()), 2), 'year': year, 'quarter': quarter}
    ...
    for month_name in dataset.month_name.unique():
        for day_ds in dataset.loc[dataset.month_name == month_name][['day', 'hash', 'month_name', 'date']].values:
            if day_ds[3] not in dates:
                report.append({'type': 'day', 'date': f'{day_ds[2]}, {day_ds[0]}', 'hash': round(date_hash_sum.get(day_ds[3]), 2)})
