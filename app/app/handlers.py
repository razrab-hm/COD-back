...
def check_token_valid(db: Session, auth: AuthJWT):
    ...
    return {"message": 'success', 'role': db_user.role, 'username': db_user.username, 'id': db_user.id}
...
def save_upload(company_id, hashrate_list, auth, db):
    ...
    if access_level == 2:
        app_companies.check_user_in_company(db, auth.get_jwt_subject(), company_id)
    return app_hashrates.upload_data(hashrate_list, db, auth.get_jwt_subject(), company_id)
...
def new_user_handler(user, db, auth):
    ...
    if app_users.get_user_by_email(db, user.email):
        raise HTTPException(409, "Email already exists")
    return app_users.new_user_with_companies(user, db, auth, access_level)


def update_company_users_handler(company_id, users_id, auth, db):
    log.input(company_id, users_id, auth, db)
    auth.jwt_required()
    access_level = app_users.get_access_level(db, auth.get_jwt_subject())
